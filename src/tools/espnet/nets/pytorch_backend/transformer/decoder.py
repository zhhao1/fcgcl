...
class Decoder(BatchScorerInterface, torch.nn.Module):
    ...
    def forward(self, tgt, tgt_mask, memory, memory_mask, args=None, flag='Train'):
        """Forward decoder.
        ...
        for i_layer, decoder_layer in enumerate(self.decoders):
            if self.dropdim and flag == 'Train':
                if args.mask_random:
                    x = mask_out(x, args.mask_prob, args.mask_value, args.mask_position)
                elif args.mask_span:
                    x = mask_block(x, args.block_number, args.K)
                else:
    ...
    def forward_one_step(self, tgt, tgt_mask, memory, cache=None, args=None):
        """Forward one step.
