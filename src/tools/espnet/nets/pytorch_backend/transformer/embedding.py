...
class embed(torch.nn.Module):
    def __init__(self, idim, attention_dim, dropout_rate, embed_type, padding_idx=-1):
        ...
        elif embed_type == "embed":
            self.embed = torch.nn.Sequential(
                torch.nn.Embedding(idim, attention_dim, padding_idx=padding_idx),
                PositionalEncoding(attention_dim, dropout_rate),
            )
        elif embde_type == "conv2d":
            self.embed = Conv2dSubsampling(idim, attention_dim, dropout_rate)
        elif embde_type is None:
            self.embed = torch.nn.Sequential(
                PositionalEncoding(attention_dim, dropout_rate)
            )
        else:
    ...
    def forward(self, xs, masks):
        if self.embed_type == 'linear' or self.embed_type == 'embed':
            xs = self.embed(xs)
        else:
