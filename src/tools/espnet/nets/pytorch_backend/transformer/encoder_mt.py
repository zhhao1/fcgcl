...
from tools.function import ctc_shrink, mask_out
...
class Encoder(torch.nn.Module):
    ...
    def forward(self, xs, masks, args=None, flag='Train'):
        ...
        for i_layer, encoder_layer in enumerate(self.encoders):
            if i_layer == 1: 
                xs, masks, x_before = encoder_layer(xs, masks, task='MT', layers=i_layer)
            else:
                xs, masks = encoder_layer(xs, masks)              
        if self.normalize_before:
            xs = self.after_norm(xs)
        return xs, masks, torch.mean(x_before, dim=1), first
